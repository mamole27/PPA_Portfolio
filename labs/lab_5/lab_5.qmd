...
```{r temporal-split}
# Split by week
# Q1 has weeks 26-39 (JUL-SEP)
# Train on weeks 26-35 (JUL 1 - early SEP)
...
# FIX ERROR: Find stations in both train and test periods
Q3_common_stations <- intersect(
  Q3_study_panel_complete %>% filter(week < 36) %>% pull(start_station) %>% unique(),
  Q3_study_panel_complete %>% filter(week >= 36) %>% pull(start_station) %>% unique()
)
# FIX ERROR: Filter BEFORE splitting
Q3_study_panel_complete <- Q3_study_panel_complete %>%
  filter(start_station %in% Q3_common_stations)
...
#FIX ERROR Convert to factor in TRAIN FIRST
Q3_train$start_station <- factor(Q3_train$start_station)
#FIX ERROR Match TEST to TRAIN levels
Q3_test$start_station <- factor(
  Q3_test$start_station,
  levels = levels(Q3_train$start_station)
)
...
# Force test stations to match training levels
# Q3_test$start_station <- factor(
#   Q3_test$start_station,
#   levels = levels(factor(Q3_train$start_station))
# )

# Keep only stations that exist in training
Q3_test <- Q3_test %>%
  filter(start_station %in% unique(Q3_train$start_station))
Q3_test$start_station <- factor(
  Q3_test$start_station,
  levels = levels(Q3_train$start_station)
)

...
2. Spatial features (create your own - https://data.boston.gov/group/geospatial
...
```{r}
schools <- st_read(here("data/Public_Schools.shp"))


# Model 1: Structural Only
mod1 <- lm(SalePrice ~ Style, 
           data = boston.sf)

# Model 2: Add Spatial Features  
mod2 <- lm(SalePrice ~ Style + dist_downtown_mi,
           data = boston.sf)


# Model 3: Add Fixed Effects
mod3 <- lm(SalePrice ~ LivingArea + Age + R_FULL_BTH +
                       crimes_500ft + crime_nn3+ dist_downtown_mi +
                       as.factor(name),
           data = boston.sf)
```
...
1. **Variables used:**
   - Structural: house style
   - Spatial: distance to public schools
   - Non-linear: sqf (diminishing returns)

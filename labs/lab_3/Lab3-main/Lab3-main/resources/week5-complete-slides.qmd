...
```{r}
#| include: false #This supresses the inclusion of this code chunk in your final quarto render
...
census_api_key(Sys.getenv("CENSUS_API_KEY"), install=TRUE, overwrite=TRUE) #If saved in your environment
#Otherwise use: census_api_key("yourkeyhere")
```
...
# Create tibble
data <- tibble(x = x, y = y, f_x = f_x, epsilon = epsilon)
...
- Null hypothesis is β₁ = 0 (population has no relationship to income)
...
- Different samples (n) of the same population (N) → slightly different blue lines
...
# percent_college + median_income as predictor
